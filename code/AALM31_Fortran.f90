PROGRAM Leg20
   ...
   ! Last revised by WGG on 30 August 2024
   IMPLICIT NONE
   !
   !  Front-end variables
   !
   CHARACTER(12), PARAMETER :: version = "30 Aug 2024 "
   ...
   REAL(8), ALLOCATABLE :: age_ts(:), buff8(:), source(:,:), adjsource(:,:)
   ...
   REAL(RK)             :: ex=1.0, adjfactor                                    ! Parameters for iterative option
   ...
   INTEGER              :: Nsource=18, Nbymedia(6)=0, activesource(18)=0        ! Source counts
   INTEGER              :: VI(0:N1)                                             ! Buffer for integer data
   REAL(RK)             :: VR(0:N1)                                             ! Buffer for real values
   CHARACTER(32)        :: var, runname                                         ! Input keyword
   ...
   REAL(RK), allocatable :: TRBCorig(:)      ! Time series for RBC without suppression
   !
   !
   ...
   call execute_command_line("mkdir -p output/"//TRIM(ADJUSTL(runname)), exitstat=ioerr)
   OutFile = "output/"//TRIM(ADJUSTL(runname))//"/RunInfo_"//TRIM(ADJUSTL(runname))//".txt"
   PRINT *, OutFile
   ...
   OutFile = "output/"//TRIM(ADJUSTL(runname))//"/Log_"//TRIM(ADJUSTL(runname))//".csv"
   OPEN(12, FILE=OutFile, IOSTAT=ioerr, ACTION="WRITE", RECL=1000)
   OutFile = "output/"//TRIM(ADJUSTL(runname))//"/Src_"//TRIM(ADJUSTL(runname))//".csv"
   OPEN(13, FILE=OutFile, IOSTAT=ioerr, ACTION="WRITE", RECL=1000)
   OutFile = "output/"//TRIM(ADJUSTL(runname))//"/Out_"//TRIM(ADJUSTL(runname))//".csv"
   OPEN(14, FILE=OutFile, IOSTAT=ioerr, ACTION="WRITE", RECL=1000)
   OutFile = "output/"//TRIM(ADJUSTL(runname))//"/Day_"//TRIM(ADJUSTL(runname))//".csv"
   OPEN(16, FILE=OutFile, IOSTAT=ioerr, ACTION="WRITE", RECL=1000)
   PRINT*, "OutFile = ", TRIM(ADJUSTL(OutFile))
   OutFile = "output/"//TRIM(ADJUSTL(runname))//"/Rates_"//TRIM(ADJUSTL(runname))//".csv"
   ...
   ALLOCATE(TRBCorig(0:NTS), STAT=allocerr)
   CALL Adjust_inputs()
   ...
   DO iteration=1, iter%maxiter
      ! Note that TS, PC, and source arrays retain old values
      Y        = 0.
      FL       = 0.
      MB       = 0.
      SrcT     = 0.
      ...
      IF (sim%iterate==0) adjust(1:18) = 1.0
      IF (sim%iterate==1) THEN
         IF (iteration==1) THEN
            scalar = 0.
            adjust(1:18) = 1.0 - REAL(adjustable(1:18))
            PRINT*, "adjust (iter 1) = ", adjust(1:18)
         ENDIF    
         IF (iteration==2) THEN
            scalar = 1.
            lastscalar = 0.
            adjust(1:18) = 1.0
            PRINT*, "adjust (iter 2) = ", adjust(1:18)
         ENDIF    
         IF (iteration>2) THEN
            ex = 1.0 
            if(sim%irbc==1) ex = 1. + BLL%center/50.    ! more curvature at higher BLL
            adjfactor = (BLL%center**ex - BLL%actual**ex) / (BLL%actual**ex - BLL%previous**ex) 
            nextscalar = scalar + (scalar-lastscalar) * adjfactor
            ...
            PRINT '(3F9.4)', lastscalar, scalar, nextscalar
            PRINT*, "adjust (iter ",iteration,")  = "; PRINT*, adjust(1:18)
            lastscalar = scalar
            scalar = nextscalar
         ENDIF 
      ENDIF
      DO J=1,18
         adjsource(J, 1:NTS) = source(J, 1:NTS) * adjust(J) 
      ENDDO    
      ...
      !  Loop over timesteps
      !
      PRINT*, " Base Ingestion on timestep 1 = "; PRINT*,    source(4:Nsource, 1)
      PRINT*, " New ingestion on timestep 1  = "; PRINT*, adjsource(4:Nsource, 1)
      ...
      DO T=1, NTS
         ...
         IF(TS(Hematocrit, T-1)>0. .AND. TS(Blood_vol, T-1)>0.) THEN
            TS(RBC_conc, T) = Y(RBC, T-1) / (TS(Hematocrit, T-1) * TS(Blood_vol, T-1))
         ENDIF
         cf = 1.
         TS(T_RBC, T) = TRBCorig(T)
         ...
         dep   = 0.
         DO K=1, 3
            inhal   = adjsource(K, T) * Lung(K)%DepFracLET 
            dep(K)  = dep(K) + inhal
         ...
         DO K=1, 3
            inhal   = adjsource(K, T) * Lung(K)%DepFracLTB 
            dep(K)  = dep(K) + inhal
         ...
         ! Lungs Alveolar (Lalv)
         DO K=1, 3
            inhal   = adjsource(K, T) * Lung(K)%DepFracLalv
            dep(K)  = dep(K) + inhal
            inmass  = inhal
         ...
         NDEP(1:3, T) = adjsource(1:3, T) - dep(1:3) 
         ...
         IF ((sim%debug==1 .AND. NTS<20*sim%outwrite) .OR. T==NTS) THEN
            ...
            DO J=1, Nbymedia(1)
               inmass   = SUM(Adjsource(J, 1:T)) - SUM(NDEP(J, 1:T))
               lungplas = 0. !SUM(YlungTM1(9:12, J))
               outmass  = SUM(SrcT(4:14, J)) + SUM(YlungT(1:12, J)) + lungplas
               PRINT '(1X,A6,A9,F14.3)', Allsource(J)%name, " Source =", SUM(adjsource(J, 1:T))
   ...
   DO J=sim%outwrite,NTS,sim%outwrite
      ...
      WRITE(13,fmt1) J, ",", age_ts(J), ",",age_ts(J)/365.,",", adjsource(1,J), ",", adjsource(2,J), ",", adjsource(3,J),  &
          ",", adjsource(4,J), ",", adjsource(5, J), ",", adjsource(8,J), ",", adjsource(7, J), ",", adjsource(8,J), ",",  &
          adjsource(9,J), ",", adjsource(10, J), ",", adjsource(11, J), ",", adjsource(12, J), ",", adjsource(13,J), ",",  &
          adjsource(14,J), ",", adjsource(15, J), ",", adjsource(16, J), ",", adjsource(17, J), ",", adjsource(18,J), ",", & 
          MB(Yin,J),",", MB(Ybody,J),",",MB(Yout,J),",",MB(Ycomp,J)+MB(Yflow,J)-MB(Yin,J) 
   ENDDO   
   fmt2 =  '(I8,31(A1,F15.5))' 
...
SUBROUTINE Read_sim_data()
   ...
   IF (Ndays>0 .AND. Nperday>0 .AND. NTS==0 .AND. sim%outwrite>=0) THEN
      ...
      ALLOCATE(adjsource(18,1:NTS), STAT=allocerr)
...
SUBROUTINE Read_media_data()
   ...
   WRITE(12,*) "Media line : ", var, vartype, nmedia, med3(nmedia)
   IF (var=="SOURCES") THEN
      n = VI(1)
      Nbymedia(nmedia) = n
      base = 3*nmedia-3
   ...
   IF(INDEX(var,"RBA")>0) THEN
      ...
      DO J=1,3
         ...
         WRITE(12,*) "Source line 1:", J, media(nmedia), first, SUM(conc(J,1:NTS))/NTS
         WRITE(12,*) "source_line2", SUM(intake_tot(1:NTS))/NTS, SUM(intake_frac(J,1:NTS))/NTS
         IF(SUM(source(first+J-1, 1:NTS)) > 0.)  Activesource(first+J-1) = 1
      ENDDO 
      WRITE(12,*) "Summary of ", media(nmedia), ": mean intake per TS = ", SUM(conc(1:3,1:NTS))/NTS 
      ! Reset concs to zero for start of next media type
      conc(:,:) = 0.
   ENDIF    
...
SUBROUTINE InputSummary()
   ...
   WRITE(15,*) 
   WRITE(12,*) " Number of sources = ", Nsource
   WRITE(12,*) " Source info: "
   DO I=1, 18
      WRITE(12,*) I, ActiveSource(I), Allsource(I)%name
   ENDDO    
   WRITE(12,*) 
...
SUBROUTINE IntakeUptake()
   ...
   IF (MOD(T,Nperday)==1 .OR. Nperday==1) THEN
       ...
       Excrete(:) = 0.
   ENDIF 
   Intakes(InairTot) = Intakes(InairTot) + SUM(adjsource(1:3, T))
   LungDep           = Lung(1:3)%DepFracLET + Lung(1:3)%DepFracLTB + Lung(1:3)%DepFracLalv
   Intakes(InairDep) = Intakes(InairDep) + SUM(adjsource(1:3, T) * LungDep)
   Intakes(InIngest) = Intakes(InIngest) + SUM(adjsource(4:18, T))
   DO J=4,Nsource
      SELECT CASE(Allsource(J)%Name(1:4))
        CASE("DUST") 
          Intakes(InDust)   = Intakes(InDust) + adjsource(J, T) 
          Uptakes(UpGIDust) = Uptakes(UpGIDust) + SrcT(F18SIPlas, J)
        CASE("SOIL")  
          Intakes(InSoil)   = Intakes(InSoil) + adjsource(J, T) 
          Uptakes(UpGISoil) = Uptakes(UpGISoil) + SrcT(F18SIPlas, J)
        CASE("WATE")
          Intakes(InWater)  = Intakes(InWater) + adjsource(J, T)  
          Uptakes(UpGIWater)= Uptakes(UpGIWater) + SrcT(F18SIPlas, J)
        CASE("FOOD")
          Intakes(InFood)   = Intakes(InFood) + adjsource(J, T)   
          Uptakes(UpGIFood) = Uptakes(UpGIFood) + SrcT(F18SIPlas, J)
        CASE("OTHE")
          Intakes(InOther)  = Intakes(InOther) + adjsource(J, T)
